...
/* 扩展数据源登录态(可选)。填手机登录 App 时那条登录响应里的 UserID + Token(长期有效),
...
function ProviderSection() {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-2">
        <label className="text-[12px] text-text-dim font-semibold">扩展数据源登录态（可选 · 深度龙虎榜）</label>
        <span className="text-[11px] font-mono text-text-muted">
          {!st ? '' : !st.configured ? '未配置' : st.valid ? `已登录 · UID ${st.uid}` : 'Token 失效'}
        </span>
      </div>
      <p className="text-[11px] text-text-muted mb-2 leading-relaxed">
        用<span className="text-[var(--color-signal-moderate)]">你自己的扩展数据源账号</span>解锁深度龙虎榜（席位带游资身份标签，比东财裸营业部名多一层）。
