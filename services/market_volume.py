"""市场量能(全天外推式): 沪/深/创业/科创 四市场 成交量+成交额, 日频历史 + 当日分时。
...
async def market_volume_intraday(market: str = "两市") -> dict:
    ...
    # 预测量能序列(全天外推式): 纯比例外推 proj(t) = 今日累计(t) ÷ 近N日该时点"完成度"中位数。
    # 完成度 = cum_d(t)/full_d(当日累计占当日全天比)。取中位数对单日异常更稳。
    # 相比旧"锚定近5日均值估剩余"的好处: anchor 被约掉, 预测自动跟随今日真实量级——
    # 清淡日不再死扛在均值上(旧法早盘 +20~37% 虚高), 放量日也不封顶。收盘=实际。
    # 前端画成相对昨日总量的 % 偏离曲线。留一回测: 午后 ±2%, 早盘 ±8~10%(5日样本天花板)。
    recent = hist[-5:]
    proj_series = []
    projected = None
    if recent and today:
        def frac_at(t, idx):
            ...
            for day in recent:
                ...
                if full > 0:
                    fr.append(cum / full)
            return statistics.median(fr) if fr else None

        for t, cv, ca in today:
            fa, fv = frac_at(t, 2), frac_at(t, 1)
    ...
    out = {"market": market, "points": points, "prev_points": prev_points,
           ...
           "note": "预测量能(全天外推式): 今日累计 ÷ 近5日该时点完成度中位数外推全天, 相对昨日总量的偏离; 收盘收敛到实际。"}
    if points:
