...
async def _tool_zsxq_digest(days: int = 1, all_authors=None) -> dict:
...
async def _tool_market_sentiment() -> dict:
    try:
        ...
        try:
            ...
            quality = None
        # 接力: 昨天各格子里的票今天怎么样了 + 连板梯队迁移。只给统计与前几只, 不铺全名单
        # (agent 要的是"接得住接不住"这个判断, 铺 40 只成分是噪音)。
        relay = None
        try:
            from services.stock_tags import (pools as _pools, migration as _mig,
                                              pool_backtest as _pbt)
            p = await _pools()
            if p.get("可用"):
                mg = await _mig(p["锚点日"])
                # 历史分池是"今天这批数值算大还是算小"的唯一标尺: 单日 28/9/7 只说明不了
                # 任何事, 245 天的均值才是参照。不可用(覆盖率不够/对照没过)时整块省掉。
                bt = await _pbt()
                relay = {"锚点日": p["锚点日"],
                         "各池": [{"池": g["池"], "只数": g["只数"], **(g["统计"] or {}),
                                   "领涨": [x["名称"] for x in g["成分"][:3]]} for g in p["池"]],
                         "梯队迁移": (mg.get("梯队迁移") if mg.get("可用") else None),
                         "历史分池均值": ({"回放天数": bt["参与统计的天数"], "分池": bt["分池"],
                                          "幅度对照通过": bt["幅度对照通过"], "口径": bt["口径"]}
                                         if bt.get("可用") else None),
                         "口径": p["口径"]}
        except Exception:
            relay = None
...
async def _tool_seat_history(q: str) -> dict:
    ...
    try:
        ...
        return await seat_history(q)
    except Exception as e:
        return {"error": f"席位追踪失败: {e}"}
...
_SYSTEM = (
    ...
    "【每个数值挂标的名】落笔的每个具体数值(价位/颈线/支撑压力位/涨跌幅/成本/量比)都紧跟它所属的标的名, 一个数配一个名; "
    "涉及多只标的时逐标的分条写('通信ETF 颈线 0.742 / 科创50ETF 颈线 1.149 / …'), 让读者对每个数字都能立刻对应到具体标的。\n"
    ...
    "  · 【'质地怎么样/是不是好生意/值得长期拿吗'——生意质量四层框架(不点名出处)】"
    "①这门生意怎么赚钱: get_company_profile 的主营构成, 一句话说清商业模式与钱从谁手里赚; 商业模式绕/看不懂的如实标'复杂生意, 超出可简单理解的范围'; "
    "②护城河有没有、是哪种: 定价权(毛利率高且多年稳)/成本优势/品牌与无形资产/转换成本/网络效应/牌照资源, 用 get_fundamentals 的毛利率/净利率/ROE 佐证, 用 get_peers 对照同行印证'比同行好在哪、好多少'——没有可佐证的护城河就说'未见明显护城河, 属同质化竞争'; "
    "③盈利质量: ROE 的水平与多年持续性(一年高不算, 持续高才是质地), 增长靠内生还是靠杠杆/并购堆出来; "
    "④价格与价值分开说: PE/PB 放进该股自身历史区间与同行水平对照, 表述为'当前估值处于什么位置(贵/中/便宜是相对历史与同行而言)'——好生意与好价格是两个独立结论, 分开给, 该不该按这个价买由用户定。"
    "get_red_flags 作为质地的反面证据一并扫; 哪层数据查不到就如实说哪层查不到。\n"
    ...
    "【单笔得失/被套/该不该认错 这类心态问题】数据分析之外, 用概率思维作参照框架(不点名出处): "
    "单笔结果是打法系统的随机样本而非成绩单, 值得复盘的是'这笔属于既定打法内的动作, 还是打法外的冲动单'; "
    "优势只在一串交易上兑现, 整体赚钱并不需要这一笔是对的; 拿不住/恐慌砍仓多半源于下单时没有预先接受这笔的风险(金额超出亏得起的范围)。"
    "以心态参照的口吻客观陈述, 结合他的实际成交数据(get_trades)对照, 决策仍由用户定。\n"
    ...
    "【指数/板块大涨大跌归因——消息面之外必须补资金面实证】问'指数/某板块昨天为什么涨那么猛/跌那么狠'时, 新闻+联网只回答'催化是什么', "
    "还要回答'钱从哪来、谁在买': get_market_sentiment(涨跌家数+涨跌分布看普涨还是结构性) + get_sector_momentum(该板块动量与量价配合) + "
    "get_lhb()(当日龙虎榜净买额榜=资金主线上了哪些票), 再对榜内 1-2 只领涨代表股调 get_lhb(code, 该日) 看席位画像——机构专用/北向/常见量化通道各占多少, "
    "直接回答'是机构在买、北向在买还是量化游资在推'。对'量化被限/关机了'这类资金结构传闻: web_search 求证消息本身, 同时用当日席位明细里量化通道席位的出现频次与金额做间接观察; "
    "龙虎榜只覆盖上榜股, 明说这是抽样证据, 把实证和推测分开表述。\n"
