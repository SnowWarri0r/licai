...
| 资金·机构 | `ProviderInstTheme`(机构增仓 + 本月热门题材) |
| 宏观 | `MacroDashboard` |
| 资讯 | `PortfolioNews` |
...
真正缺的是**未知 key 兜底**:当前从 hash 读到任何字符串都直接当 view 用,遇到不在合法集合里的值(例如缺陷 §七 写入的 `dashboard`,或手输错的 hash)就渲染出空白内容区。因此:

- 定义合法 view key 集合,初始化读 hash 时校验,不在集合内则回落到 `portfolio`。
- 这一条同时兜住了 §七 的缺陷,但 §七 那个错误调用仍要单独改对(不靠兜底掩盖)。
...
3. **hash 回归**:访问 `#sector` `#review` 等保留的老 key,确认页面正常(内容变少是预期);访问 `#dashboard` 和一个随手编的 `#nonsense`,确认都兜底到持仓页而非空白。
4. **后端回归**:本次是纯前端改动,跑一遍后端测试(基线 665 passing)确认未被牵连。
5. 死代码删除后 `grep` 全量确认无残留引用。

## 风险

- 榜单合并要改 `Rankings` 的 tab 机制,该文件 1120 行且含持久化的 tab 状态(`useState(() => ...)` 读取已存 tab),新增 tab 时需兼容已存的旧 tab 值。
