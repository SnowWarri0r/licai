...
预期 `✓ built`。常见错误:某个组件的 import 被上一步误删 → "X is not defined";把 import 段与实际使用对齐即可(`MorningBriefing` `SentimentThermometer` `SectorShare` `SectorMatrix` `SectorOpportunities` `SectorRadar` `ProviderInstTheme` `EtfXray` `BenchmarkCompare` `Cashflow` `AllocationAdvisor` `AShareSectorGap` `PortfolioCurve` `PortfolioCorrelation` `AITradeReview` 全部仍需保留)。
