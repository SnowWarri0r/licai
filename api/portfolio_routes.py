...
async def trade_review_ai(period: str = "all", force: int = 0):
    ...
    if period != "all":
        ...
        system_prompt = (
            ...
            "资金形态与打法匹配——高波动/高风险打法(做T/短线/题材)用固定资金(每次投入固定、盈利移出战场, 输赢不改变下次投入), "
            "复利滚仓只配低回撤系统; 高风险打法越赢越加码属资金形态错配。\n"
            "另一组是概率思维(不点名出处): 单笔输赢只是系统的正常样本, 复盘评的是'这笔是既定打法内的动作, 还是打法外的冲动单', 而非单笔对错; "
            "一致性看整段——同类场景是否同样处理, 连续亏损后打法有没有变形(报复性交易/突然放大金额/该出手时不敢动); "
            "恐慌卖出或拿不住, 多半说明下单时没有预先接受这笔的风险(金额大到亏不起, 或压根没想过会亏); "
            "要求每笔都对是最普遍的心态漏洞——优势只在一串交易上兑现, 把自尊绑在单笔上是变形的起点。这组对照也写进 binchuan 数组。\n"
    ...
    system_prompt = (
        ...
        "  · 资金形态匹配打法: 高收益高风险打法用固定资金增值(单次投入固定、盈利抽离), 复利资金增值留给低回撤系统; "
        "长久的交易是系统的艺术, 越赢越加码地把复利滚进高波动打法=资金形态错配\n"
