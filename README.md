...
左侧榜单 + 右侧 K 线，↑↓ 键翻股、←→ 切分类，想问再唤起 AI。头部还有**自由查股搜索框**：输入代码 / 名称 / 子串出候选（带实时涨跌），选中即进右侧同一套面板——不限榜单里有没有它，想查啥查啥：

- **自选观察池**：在看但未必持有的票。任意榜单选中股票点右上 ☆ 加入（记下当时价格），自选页每行给 今日涨跌 / **自选以来涨跌** / **当下 K 线结构形态**（阶梯式上行 / 头肩顶 / 2B 假突破…每天看结构还完不完好）/ 业绩预告徽章
- **盘口异动**（同花顺式事件流）：交易所 22 类异动事件——火箭发射 / 高台跳水 / 大笔买卖 / 封板·开板 / **竞价异动**（9:15-9:25 高开低开、竞价抢筹压价）/ 缺口 / 60日新高低，盘中 60s 自动滚动。事件按 拉升/跳水/竞价 分组，组内还能按具体类型细分（chip 带全天计数，←→ 方向键循环切换）；每行给按类型解析的**关键幅度**（急变速度 / 封单手数 / 大单金额）+ 反复触发的**「今日 n 次」角标**（烂板反复挣扎一眼可见）。底部三件套：**今日最活跃**（全天反复异动 top6，点名字进 K 线）、**全天脉搏分布图**（5 分钟一档拉升红/跳水绿双向柱，看全天多空节奏）、近 30 分钟拉升 vs 跳水计数
...
K 线本体是券商级可缩放蜡烛图（量 + MA5/10/20/30/60 彩色图例 + 昨收线 + 跳空缺口阴影 + 悬浮涨跌%），**点任意蜡烛出「分时›」提示，再点弹出浮层**：该日**分时图**（当日实时 + 历史分时，走[通达信数据源](https://github.com/SnowWarri0r/tdx-api)）与**龙虎榜席位**两个页签；某日没上榜时直接列出这只票**最近的上榜日**，窗口内的一键跳转。历史分时的昨收基准做了**除权精确还原**——K 线是前复权价、分时是当日真实成交价，除息日之后回看会错位出"主板 +13%"的假涨跌，这里用分红送配明细逐事件逆变换（派息加法 / 送转乘法），基准标注"除权校正·精确"；事件表拉不到时退近似口径并如实标注。任意一只点「问 AI」唤起对话抽屉，agent 自取数据后客观解读，跟"问问市场"同一套工具。
...
- 数据源可插拔：**分时 / 周月 K 线 / 五档盘口 / 逐笔成交**这四样走自部署的通达信协议服务 [SnowWarri0r/tdx-api](https://github.com/SnowWarri0r/tdx-api)（Docker 起一个，把 `tdx_base_url` 配成它的地址，或用环境变量 `TDX_BASE_URL`）。不启用也能用，只是详情页收敛成只有「日」一个页签——实时行情、日线、看板与榜单全都不依赖它
...
- 历史分时（K线浮层）：通达信协议（可选，本项目对着 [SnowWarri0r/tdx-api](https://github.com/SnowWarri0r/tdx-api) 这个 fork 测的——比上游多了 588/56x 场内 ETF 的市场前缀修复、`kline-history` 日期范围修复和可配端口；用上游也能跑，licai 侧一律显式带 `sh/sz/bj` 前缀请求）；昨收基准的除权还原用 东财分红送配明细（akshare）
...
### 可选：通达信数据源（分时 / 周月K / 盘口 / 逐笔）

个股详情页的 **分时、周/月 K 线、五档盘口、逐笔成交**这四样需要一个本地通达信协议服务。本项目对着 [SnowWarri0r/tdx-api](https://github.com/SnowWarri0r/tdx-api) 这个 fork 测（比上游多了 588/56x 场内 ETF 市场前缀、`kline-history` 日期范围、可配端口三处修复；用上游也能跑，licai 侧请求一律显式带 `sh`/`sz`/`bj` 前缀，绕开了上游按代码长度推断市场那个坑）。
