...
| **纠正沉淀成规则** | 从历史对话里挖"用户在纠正我"的三元组（问题 → 回答 → 纠正），起草成候选规则**进待审队列**。`python scripts/rules.py --mine / --approve N / --reject N`——**批准了才进 prompt**，pending 的一律不生效 |
| **工具缺口台账** | 工具返回报错 / 空 / 过期（按交易日算，不按日历天）/ 关键字段全 0 时记一笔，`python scripts/tool_gaps.py` 看排行——"哪个市场缺哪类数据"有账可查，而不是等下次答错 |

闸门是刻意的：prompt 对单个词都敏感（曾因词表里缺"主题/标题"两个词让模型吞掉整句首句），自动往里加规则最容易造成规则互撞。所以只自动**提案**，不自动生效。
...
- **「自选」是一个真实分组**，跟自建的组平级，只有两处特殊：☆ 加进来时默认落它、取消到一个组都不剩时退回它（否则这只票只在「全部」里看得到，等于丢了）
