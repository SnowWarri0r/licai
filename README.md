...
- **分时图** — 当日实时 + 历史分时，走[通达信数据源](https://github.com/SnowWarri0r/tdx-api)；可切「逐笔精绘」用全天逐笔还原分钟内的秒级尖峰（盘口被打空的「闪电」）
...
- **当日分时**：价 / 均价线 + 昨收基准 + 成交量按主动买卖着色（红买绿卖）+ 09:30 开盘点。Y 轴按当日真实高低自适应铺满（不做昨收对称留白，高开高走/大跌日的起伏照样占满可用高度）
