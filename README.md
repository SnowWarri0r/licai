...
- 服务端已经不记得这一轮了（跑完太久被清 / 服务重启过）→ 回库里取原文，取不到才标「这一轮没跑完」，不让界面一直转
...
榜单一行原来只有一级行业（「元件」「通信设备」），看不出今天涨的是 CPO 还是 PCB——**轮动发生在概念这一层**。所以把同一张榜按概念/行业聚成堆，铺在右栏（没选股票时那片本来是空的）：
...
- **穿透同源风险**：把持有的基金/ETF 拆到季报前十大，把直持个股与基金间接持有的同一标的合起来算，再按行业归并
  ...
  - **口径写在结论旁边**：前十大合计只占基金净值 25%~50%，所以给的是**下限**；拉不到明细的（联接基金/债基）单独列出，不当成敞口为 0
...
│   ├── ask_runs          # 一轮 agent 的生命周期(跑在服务端, 切页/刷新都不打断 + 自动落库)
│   ├── trade_fills       # 分笔成交合并(一次委托的多笔成交按一次决策算)
│   ├── exposure          # 穿透敞口(基金拆到季报前十大 → 同标的/同行业合并 → 同源风险)
